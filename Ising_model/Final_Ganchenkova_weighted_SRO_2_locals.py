...
def runIsingModel_SRO_2(numberOfSwaps):
    ...
    for i in range(numberOfSwaps):
        ...
        if(i%saveFrequency==0 and i>2):
            #writeGraphToXYZ(size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_2_'+str(SROy2[-1])+'SwapsSoFar'+str(SwapsSoFar)+'.xyz')
            pass
...
def getSRO_2():
    ...
    for node in H:
        if H.node[node]['species'] == 'Ga':
            neighbours = getNeighbours(node,2,H)
            numberOfIndiumNeighbours = 0
...
def graphSRO_2():
    ...
    plt.plot(SROx,SROy2)
    plt.xlabel('Step')
    plt.ylabel('SRO_2_'+str(indiumComposition)+'_'+str(T)+'K')
    plt.show()
...
def main():
    ...
    ###############################################################################
    runIsingModel_SRO_2(steps)
    graphSRO_2()
    printSRO_2()
    a = numpy.asarray([SROx,SROy2])
    numpy.savetxt('CSV_'+size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_2'+'.csv', a, delimiter=",")
    #writeGraphToXYZ(size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_2_'+str(SROy2[-1])+'.xyz')
    #nx.write_gpickle(G,size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_2_'+str(SROy2[-1])+'.gpickle')
    #pr.disable()
    #s=StringIO.StringIO()
