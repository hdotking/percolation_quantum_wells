...
def runIsingModel_SRO_3(numberOfSwaps):
    ...
    for i in range(numberOfSwaps):
        ...
        if(i%saveFrequency==0 and i>2):
            #writeGraphToXYZ(size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_3_'+str(SROy3[-1])+'SwapsSoFar'+str(SwapsSoFar)+'.xyz')
            pass
...
def getSRO_3():
    ...
    for node in H:
        if H.node[node]['species'] == 'Ga':
            neighbours = getNeighbours(node,3,H)
            numberOfIndiumNeighbours = 0
...
def graphSRO_3():
    ...
    plt.plot(SROx,SROy3)
    plt.xlabel('Step')
    plt.ylabel('SRO_3_'+str(indiumComposition)+'_'+str(T)+'K')
    plt.show()
...
def main():
    ...
    ###############################################################################
    runIsingModel_SRO_3(steps)
    graphSRO_3()
    printSRO_3()
    a = numpy.asarray([SROx,SROy3])
    numpy.savetxt('CSV_'+size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_3'+'.csv', a, delimiter=",")
    #writeGraphToXYZ(size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_3_'+str(SROy3[-1])+'.xyz')
    #nx.write_gpickle(G,size+'_xIn='+str(indiumComposition)+'_T='+str(T)+' K_SRO_3_'+str(SROy3[-1])+'.gpickle')
    #pr.disable()
    #s=StringIO.StringIO()
