    ...
    def run_Ising_model_sro1(self, num_steps):
        ...
        for i in range(num_steps):
            ...
            if self.attempt_atom_swap():
                successfulSwaps += 1
            if (i % self.statsFrequency == 0):
                print(float(i) / ns * 100, '% Done')
                self.SROx.append(i)
                self.SROy1.append(self.getSRO_1())
    ...
    def getSRO_1(self):
        ...
        for node in self.H:
            if self.H.nodes[node]['species'] == 'Ga':
                ...
                neighbours = self.getNeighbours(node, 1)
                numberOfIndiumNeighbours = 0
...
# but might fluctuate from site to site, and therefore indiumCount would be appended by 0.5 on average
    ...
    def getNeighbours(self, node, weight):
     ...
     # i want to be able to input a node and a weight and get all the neighbours pos
        ...
        # checke if we already have this key (reduce time complexity)
        key = (node, weight)
    ...
    def save_output(self):
        a = numpy.asarray([self.SROx, self.SROy1])
        numpy.savetxt('CSV_' + self.size + '-xIn=' + str(self.In_composition) + '_T=' + str(self.T) + 'K_SRO_1' + '.csv', a,
                      delimiter=",")
        self.writeGraphToXYZ(self.size + '-xIn=' + str(self.In_composition) + '_T=' + str(self.T) + 'K_SRO_1=' + str(self.SROy1[-1]) + '.xyz')
        nx.write_gpickle(self.G, self.size + '-xIn=' + str(self.In_composition) + '_T=' + str(self.T) + 'K_SRO_1=' + str(
